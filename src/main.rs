...
// - bool, f32
// - crossover example
// - with reorder, dag, etc.
// standard bool with json
...
fn bioma_uci(args: Cli) {
    let (train_data, train_label, eval_data, eval_label) = match args.dataset_args.dataset.as_str() {
        "abalone" => real_world_uci::abalone::get_dataset(r"C:\Users\Henning\IdeaProjects\cgp_master\src\datasets\real_world_uci\data\abalone.data".to_string(), false),
        "cancer" => real_world_uci::breast_cancer::get_dataset(r"C:\Users\Henning\IdeaProjects\cgp_master\src\datasets\real_world_uci\data\breast+cancer+wisconsin+diagnostic.data".to_string(), false),
        "page" => real_world_uci::page_blocks::get_dataset(r"C:\Users\Henning\IdeaProjects\cgp_master\src\datasets\real_world_uci\data\page-blocks.data".to_string(), false),
        _ => { panic!("Unknown dataset type {}", args.dataset_args.dataset_type) }
    };
...
fn main() {
    ...
    // bioma_f32(args);
    bioma_uci(args);
    // if args.dataset_args.dataset_type == "bool" {
    //     bioma_bool(args);
    // } else {
    //     bioma_f32(args);
    // }
